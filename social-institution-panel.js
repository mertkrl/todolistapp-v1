...
//
// Köprüler:
//  - renderMyInstitutionModal, loadMyGroupsSupabase: social.js'in geri kalanı
//    tarafından window.* ile çağrılıyor (bu modülde tanımlı, dışa açıldı).
...
import {
    ...
    async function renderMyInstitutionModal() {
    ...
    async function loadMyGroupsSupabase() {
        ...
        const renderList = async () => {
            ...
            if (!rows || rows.length === 0) {
                ...
                if (currentActiveGroupCode) window.resetActiveGroupPanel();
            ...
            for (const row of rows) {
                ...
                const groupData = await _normalizeSupabaseGroup(groupRow, memberRows || []);
                ...
                groupCard.classList.toggle("active-hub-group", currentActiveGroupCode === groupCode);
                ...
                groupCard.innerHTML = `
                    <div class="my-group-card-row">
                        ${groupAvatarHtml(groupCode, groupData.name, 30)}
                ...
                groupCard.onclick = function() {
                    currentActiveGroupCode = groupCode;
                ...
                if (!currentActiveGroupCode && groupCode === firstGroupCode) {
                    currentActiveGroupCode = groupCode;
                    groupCard.classList.add("active-hub-group");
                }
            }
       ...
       async function openGroupInviteModal(code, data) {
        ...
        listEl.querySelectorAll(".group-invite-send-btn").forEach(btn => {
            btn.onclick = async () => {
                ...
                try {
                    ...
                    dcShowToast(`@${targetUsername} kullanıcısına davet gönderildi.`);
                } catch (e) {
                    btn.disabled = false;
                    dcShowToast('Davet gönderilemedi: ' + e.message);
                }
            };
        });
       }
       ...
       async function _cpGenerateStudentReport({ data, isWork, memberLabel, assignments, subsByAsg, subGrades, stepDoneByAsg, submittedAtByAsgUser, scheduleRows, DAY_NAMES_TR, studentUserId, studentName }) {
