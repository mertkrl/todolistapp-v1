...
(function () {
    ...
    // CSP ihlali otomatik tespiti — tarayıcı CSP'deki herhangi bir kuralı (script-src,
    // style-src'deki 'unsafe-hashes' dahil) engellediğinde bu olay tetiklenir. Böylece
    // 'unsafe-hashes' gibi yeni/az desteklenen direktiflerin gerçek kullanıcı
    // tarayıcılarında sorun çıkarıp çıkarmadığını konsolu izlemeden görebiliriz —
    // sessizce hiçbir stil/script engellenmiyor gibi görünüp aslında engelleniyor
    // olması ihtimaline karşı bir erken-uyarı sistemi.
    document.addEventListener('securitypolicyviolation', function (ev) {
        console.warn('[FocusAI] CSP ihlali:', ev.violatedDirective, ev.blockedURI);
        report('csp_violation', ev.violatedDirective + ' | blocked: ' + ev.blockedURI + ' | sample: ' + (ev.sample || '').slice(0, 200), null);
    });
})();
